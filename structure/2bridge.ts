{
  ...
  // 구현계층 ~ 인터페이스 적용한 하위 클래스 ~ 역할 분담에 따라 기능 클래스를 복수개 생성 & 기능은 커스텀해서 구현
...
/**
 * 구현부와 추상부의 분리
 *  추상부 ~ high level layer ~ UI
 *  구현부 ~ 내부의 실제 코드 ~ 비즈니스 로직 (바깥으로부터 숨겨짐)
 * 
 *  추상부와 구현부를 인터페이스로 연결만 시켜 놓으면,
 *   추상부의 확장 용이, 구현부의 딮한 내용 이해 필요X
 *   구현부도 추가 용이 (인터페이스만 지키면 된다!)
 */ 
